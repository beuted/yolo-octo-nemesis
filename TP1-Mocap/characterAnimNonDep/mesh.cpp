...
void Mesh::load(const char* fileName) {
	...
	while(fgets(line, 300, fdat)) {
		...
		if (strstr(line, "g ")) {
			...
			char* key = strtok_r_dosunix(line, " \t\n\r", &next_token);
			if (!key)
				continue;
			if (key[0]=='v' && key[1]=='n') {
				glm::vec3 vi;
				char* valx = strtok_r_dosunix(NULL, " \t\n\r", &next_token);
				char* valy = strtok_r_dosunix(NULL, " \t\n\r", &next_token);
				char* valz = strtok_r_dosunix(NULL, " \t\n\r", &next_token);
				...
				char* val = strtok_r_dosunix(NULL, " \t\n\r", &next_token);
				std::vector<unsigned int> face;
				for(;val!=NULL;val = strtok_r_dosunix(NULL, " \t\n\r", &next_token)) {
